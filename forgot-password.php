...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($user) {
    ...
    try {
      ...
      $mail->setFrom('22ct19nishanth@gmail.com', 'no-replay@eagri.ct.ws'); // Sender email and name
      ...
      if (createPassResetToken($user['userId'], $token)) {
        $resetLink = "http://eagri.ct.ws/eAuction/public/reset-password.php?user=" . $user['userId'] . "&token=" . $token;
        $mail->Body = "
          <html>
          <head>
              <style>
